...
class VK:
    ...
    def formatting_users_info_and_photos_with_likes(self):
        ...
        dict_values = {}
        for info in self.users_info()['response']:
            id_integer = info['id']
            dict_with_users_info[id_integer] = dict_values
            dict_values['first_name'] = info['first_name']
            dict_values['last_name'] = info['last_name']
            dict_values['sex'] = self.formatting_sex(info['sex'])
            try:
                dict_values['age'] = str(self.calculate_age(info['bdate'])) #обход ошибки юзеров, кто не указывал дату рождения
            except KeyError:
                dict_values['age'] = "Возраст был скрыт пользователем"
            dict_values['city'] = info['city']['title']
            dict_values['relation'] = self.formatting_marital_status(info['relation'])
            ...
            try:
                for photo in self.get_photos_from_profile()['response']['items']:
                    list_photo = []
                    list_likes = []
                ...
                for wall_photo in self.get_photos_from_wall()['response']['items']:
                    l_photo = []
                    l_likes = []
    ...
    def find_users_woman(self):
        ...
        for info in self.users_info()['response']:
            city = info['city']['id']
            try:
                age = self.calculate_age(info['bdate']) #обход ошибки юзеров, кто не указывал дату рождения
            except KeyError:
    ...
    def formatting_find_users_woman(self):
        dict_with_woman = {}
        woman = self.find_users_woman().get('response', {}).get('items', [])
        for w in woman:
            id_integer = w.get('id')
            ...
            try:
                for photo in woman_photo.get('response', {}).get('items', []):
                    list_photo_likes_id = []
                    photo_url = photo['sizes'][-1]['url']
                    likes = photo['likes']['count']
                    id_photo_p = photo['id']
                    ...
                    dict_with_woman[id_integer]['photo'].append(list_photo_likes_id)
            except KeyError:
                return 'error'
    ...
    def formatting_find_users_men(self):
        dict_with_men = {}
        men = self.find_users_men().get('response', {}).get('items', [])
        for m in men:
            id_integer = m.get('id')
            dict_with_men[id_integer] = {
                ...
                'bdate': m.get('bdate', ''),
                'city': m.get('city', ''),
                'favorite_music': m.get('favorite_music', ''),
            ...
            try:
                for photo in men_photo.get('response', {}).get('items', []):
                    list_photo_likes_id = []
                    photo_url = photo['sizes'][-1]['url']
                    likes = photo['likes']['count']
                    id_photo_p = photo['id']
                    ...
                    list_photo_likes_id.append(f'лайки {likes} ')
                    list_photo_likes_id.append(f'фото id {id_photo_p} ')
                    dict_with_men[id_integer]['photo'].append(list_photo_likes_id)
            except KeyError:
                return 'error'
